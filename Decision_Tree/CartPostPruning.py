...
def createCartTree(dataSet, testDataSet, labels, numThreshold=1, giniThreshold=0.2):
    ...
    if len(dataSet) < numThreshold:
        ...
        if majorClass in testValueCounts:
            ...
            accuracyNow = 0
        return majorityCnt(classList), accuracyNow, True
    ...
    # 当数据集的基尼指数小于预定阈值时，结束递归
        ...
        return majorityCnt(classList), 0, True
        ...
        # 创建左子树
        trueTree, trueAccuracy, trueNeedToCut = createCartTree\
            (trueSubDataSet, trueSubTestDataSet, trueLabels,
             numThreshold=numThreshold, giniThreshold=giniThreshold)
        ...
        if len(falseSubDataSet) == 0:
            ...
            falseTree, falseAccuracy, falseNeedToCut = createCartTree\
                (falseSubDataSet, falseSubTestDataSet, falseLabels,
                 numThreshold=numThreshold, giniThreshold=giniThreshold)

        # 计算划分子树后的正确率
        subAccuracy = (trueAccuracy * len(trueSubTestDataSet) +
                       falseAccuracy * len(falseSubTestDataSet)) / len(testDataSet)

        # 当左右子树均为叶子结点时，判断当前节点是否需要剪枝，否则则不剪枝，继续递归构建子树
        if trueNeedToCut and falseNeedToCut:
            # 当划分后的子树的正确率更高，继续递归创建子树
        ...
        return cartTree, accuracyNow, needToCut
