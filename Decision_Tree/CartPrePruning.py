...
def calcSubDataSetAcc(dataSet):
    classList = [row[-1] for row in dataSet]
    classCount = countLst(classList)
    majorClass = majorityCnt(classList)

    accuracy = classCount[majorClass] / len(dataSet)
    return accuracy
...
def createCartTree(dataSet, testDataSet, labels, numThreshold=1, giniThreshold=0.2):
    ...
    # 当数据集的基尼指数小于预定阈值时，结束递归
        ...
        # 计算当前准确率
        nowAccuracy = calcSubDataSetAcc(testDataSet)
        # 计算划分以后的准确率
        trueTestDataSet, falseTestDataSet = splitData(testDataSet, bestFeature, bestValue)
        if len(trueTestDataSet) != 0:
            trueTestAccuracy = calcSubDataSetAcc(trueTestDataSet)
        else:
            trueTestAccuracy = 0
        if len(falseTestDataSet) != 0:
            falseTestAccuracy = calcSubDataSetAcc(falseTestDataSet)
        else:
            falseTestAccuracy = 0
        nextAccuracy = (trueTestAccuracy * len(trueTestDataSet) +
                        falseTestAccuracy * len(falseTestDataSet)) / len(testDataSet)

        # 判断划分之后准确率是否提升，若提升则继续划分子树，否则直接返回叶子结点
        if nowAccuracy < nextAccuracy:
            # 递归构建子树
            cartTree['%s_%s'%(bestFeatureLabel, str(bestValue))] = \
                {True : None, False : None}
            cartTree['%s_%s'%(bestFeatureLabel, str(bestValue))][True] = createCartTree\
                (trueSubDataSet, trueTestDataSet, trueLabels, numThreshold, giniThreshold)

            # 当匹配为False的子树没有数据时，返回当前（在划分True和False子集之前的数据集中）最有可能的class，否则递归构建子树
            if len(falseSubDataSet) == 0:
                cartTree['%s_%s'%(bestFeatureLabel, str(bestValue))][False] = \
                    majorityCnt(classList)
            else:
                cartTree['%s_%s'%(bestFeatureLabel, str(bestValue))][False] = createCartTree\
                    (falseSubDataSet, falseTestDataSet, falseLabels, numThreshold, giniThreshold)
            return cartTree
        else:
            return majorityCnt(classList)
